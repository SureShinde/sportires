...
class QuoteManagement extends \Magento\Quote\Model\QuoteManagement
{
    ...
    protected function submitQuote(QuoteEntity $quote, $orderData = [])
    {
        ...
        $logger->info('SAVE QUOTE TO ORDER'); 
        $logger->info($quote->getId()); 
        $logger->info($quote->getCustomerEmail()); 
        //$logger->info('GUEST '.print_r($quote->getCustomerIsGuest(),true)); 
        (int)$isGuest = $quote->getCustomerIsGuest();

        $logger->info('GUEST '.$isGuest);
        ...
        $logger->info('3');
        $logger->info('PASO LA VALIDACION DE LA DIRECCION'); 
        $addresses = [];
